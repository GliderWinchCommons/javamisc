   ...
   // ======== Endless loop ====================================================
     ...
     //         System.out.format("%s\n",msg);
           ...
           if (can1.id == 0x40800000){ /* Is this CAN ID an engine sensor message? */
             eng[1] = can1.get_1int(1);   //  Extract bytes from CAN payload
             engd[1] = Float.intBitsToFloat(eng[1]); // Convert Manifold pressure
           }
           if (can1.id == 0x40A00000){ /* Is this CAN ID an engine sensor message? */
             eng[0] = can1.get_1int(1);   //  Extract bytes from CAN payload
             engd[0] = Float.intBitsToFloat(eng[0]); // Convert rpm
           }
